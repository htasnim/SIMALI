...
class SimStats {
 ...
 public:
  ...
  int64_t num_lungtissue = 0;
  int64_t num_infected = 0;
  
  
  int64_t num_aircell_w_inflam = 0;
  int64_t num_lungtissue_w_inflam = 0;
  ...
  string to_str(int width) {
	...
	totals.push_back(reduce_one(num_lungtissue, op_fast_add, 0).wait());
	...
	totals.push_back(reduce_one(num_lungtissue_w_inflam, op_fast_add, 0).wait());
...
void seed_infection(Tissue &tissue, int time_step) {
   ...
   */
    ...
    std::size_t max_iterations = inside_points.size() * 0.01; // % of inside points
    ...
    num_of_FOIs++;
       ...
       //if (num_success_inside==35){
         //break;
        //}
        if (num_of_FOIs >= max_iterations){
          break;
        }
	  //}
	 }
...
void sample(int time_step, vector<SampleData> &samples, int64_t start_id, ViewObject view_object) {
  ...
  switch (view_object) {
    ...
    case ViewObject::EPITYPE: header_oss << "epicell_type"; break;
  ...
  for (int64_t i = 0; i < samples.size(); i++) {
    ...
    switch (view_object) {
      ...
      case ViewObject::INFLAM_SIGNAL_CELL:
       ...
       case ViewObject::EPITYPE:
        if (sample.has_epicell) {
          switch (sample.epicell_type) {   // you added this field to SampleData
            case EpiCellType::TYPE1:        val = 1; break;
            case EpiCellType::TYPE2:        val = 2; break;
            case EpiCellType::AIR:          val = 3; break;
            case EpiCellType::INTERSTITIAL: val = 4; break;
            default:                        val = 0; break;
          }
        }
        break;
      ...
      for (int y = 0; y < _grid_size->y && !done; y += _options->sample_resolution) {
        for (int z = 0; z < _grid_size->z; z += _options->sample_resolution) {
          ...
          if (i >= start_id) {
            ...
            array<int, 4> epicell_type_counts{0};  // TYPE1, TYPE2, AIR, INTERSTITIAL
                ...
                for (int subz = z; subz < z + _options->sample_resolution; subz++) {
                  ...
                  if (sub_sd.has_epicell) {
                    ...
                    switch (sub_sd.epicell_status) {
                     ...
                     switch (sub_sd.epicell_type) {
                      case EpiCellType::TYPE1:        epicell_type_counts[0]++; break;
                      case EpiCellType::TYPE2:        epicell_type_counts[1]++; break;
                      case EpiCellType::AIR:          epicell_type_counts[2]++; break;
                      case EpiCellType::INTERSTITIAL: epicell_type_counts[3]++; break;
                      default: break;  // EPITHELIAL / other
                      }
                  }
            ...
            EpiCellType epi_type = EpiCellType::AIR;  // sensible default
            if (epicell_found) {
                int max_type_i = 0, max_type_count = 0;
                for (int j = 0; j < 4; j++) {
                    if (max_type_count < epicell_type_counts[j]) {
                      max_type_count = epicell_type_counts[j];
                      max_type_i = j;
                    }
                }
            switch (max_type_i) {
              case 0: epi_type = EpiCellType::TYPE1;        break;
              case 1: epi_type = EpiCellType::TYPE2;        break;
              case 2: epi_type = EpiCellType::AIR;          break;
             case 3: epi_type = EpiCellType::INTERSTITIAL; break;
            }
          }
...
void run_sim(Tissue &tissue) {
  ...
  for (int time_step = 0; time_step < _options->num_timesteps; time_step++) {
    ...
    for (auto grid_point = tissue.get_first_active_grid_point(); grid_point;
      ...
      if (!grid_point->inflam_signal_all && grid_point->chemokine > 0) {
		...
		if (grid_point->epicell && grid_point->epicell->type == EpiCellType::TYPE1){
		  ...
		  _sim_stats.num_lungtissue_w_inflam++;
		}
    ...
    if (_options->sample_period > 0 &&
      ...
      sample(time_step, samples, start_id, ViewObject::EPITYPE);
      sample_timer.stop();
    }
...
int main(int argc, char **argv) {
  ...
  Tissue tissue;
  tissue.log_epicell_type_counts();
