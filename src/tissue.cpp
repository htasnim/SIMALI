...
Tissue::Tissue()
  ...
  barrier();
  write_epicell_chunk_counts_csv(_options->output_dir + "/epicell_chunks_100.csv");
}
...
void Tissue::log_epicell_type_counts() {
  // local counts on this rank
  int64_t local_type1        = 0;
  int64_t local_type2        = 0;
  int64_t local_air          = 0;
  int64_t local_interstitial = 0;

  for (auto &gp : *grid_points) {
    if (!gp.epicell) continue;

    switch (gp.epicell->type) {
      case EpiCellType::TYPE1:
        local_type1++;
        break;
      case EpiCellType::TYPE2:
        local_type2++;
        break;
      case EpiCellType::AIR:
        local_air++;
        break;
      case EpiCellType::INTERSTITIAL:
        local_interstitial++;
        break;
      default:
        break; // EPITHELIAL / anything else
    }
  }

  // global reductions
  auto total_type1 =
      reduce_one(local_type1, op_fast_add, 0).wait();
  auto total_type2 =
      reduce_one(local_type2, op_fast_add, 0).wait();
  auto total_air =
      reduce_one(local_air, op_fast_add, 0).wait();
  auto total_interstitial =
      reduce_one(local_interstitial, op_fast_add, 0).wait();

  if (!rank_me()) {
    SLOG("EpiCell type counts: "
         "TYPE1=",        total_type1,
         " TYPE2=",       total_type2,
         " AIR=",         total_air,
         " INTERSTITIAL=", total_interstitial,
         "\n");
  }
}

void Tissue::write_epicell_chunk_counts_csv(const std::string &fname) {
    const int chunk = 100;
    ...
    int num_cx = (Nx + chunk - 1) / chunk;
    int num_cy = (Ny + chunk - 1) / chunk;
    int num_cz = (Nz + chunk - 1) / chunk;

    struct Counts {
        int64_t type1        = 0;
        int64_t type2        = 0;
        int64_t air          = 0;
        int64_t interstitial = 0;
    };

    // Local per-chunk counts on each rank
    std::map<std::tuple<int,int,int>, Counts> local;

    for (auto &gp : *grid_points) {
        if (!gp.epicell) continue;

        int cx = gp.coords.x / chunk;
        int cy = gp.coords.y / chunk;
        int cz = gp.coords.z / chunk;

        auto &c = local[std::make_tuple(cx,cy,cz)];
        switch (gp.epicell->type) {
            case EpiCellType::TYPE1:        c.type1++;        break;
            case EpiCellType::TYPE2:        c.type2++;        break;
            case EpiCellType::AIR:          c.air++;          break;
            case EpiCellType::INTERSTITIAL: c.interstitial++; break;
            default: break;
        }
    }

    // Rank 0 creates the CSV with header
    if (!rank_me()) {
        std::ofstream ofs(fname);
        if (!ofs) SDIE("Could not open CSV file ", fname);
        ofs << "cx,cy,cz,"
            << "type1,type2,air,interstitial,total,"
            << "type1_pct,type2_pct,air_pct,interstitial_pct\n";
    }
    barrier();

    // Reduce and append rows
    for (int cx = 0; cx < num_cx; cx++) {
        for (int cy = 0; cy < num_cy; cy++) {
            for (int cz = 0; cz < num_cz; cz++) {

                auto key = std::make_tuple(cx,cy,cz);
                Counts local_c{};
                if (local.count(key)) local_c = local[key];

                int64_t t1 = reduce_one(local_c.type1,        op_fast_add, 0).wait();
                int64_t t2 = reduce_one(local_c.type2,        op_fast_add, 0).wait();
                int64_t ar = reduce_one(local_c.air,          op_fast_add, 0).wait();
                int64_t it = reduce_one(local_c.interstitial, op_fast_add, 0).wait();

                int64_t total = t1 + t2 + ar + it;

                double p1 = 0.0, p2 = 0.0, par = 0.0, pit = 0.0;
                if (total > 0) {
                    p1  = 100.0 * (double)t1 / (double)total;
                    p2  = 100.0 * (double)t2 / (double)total;
                    par = 100.0 * (double)ar / (double)total;
                    pit = 100.0 * (double)it / (double)total;
                }

                if (!rank_me()) {
                    std::ofstream ofs(fname, std::ios::app);
                    if (!ofs) SDIE("Could not append CSV file ", fname);
                    ofs << cx << "," << cy << "," << cz << ","
                        << t1 << "," << t2 << "," << ar << "," << it << ","
                        << total << ","
                        << p1 << "," << p2 << "," << par << "," << pit << "\n";
    ...
    barrier();
    if (!rank_me()) {
        SLOG("Wrote epicell 100^3 chunk counts (with percentages) to ", fname, "\n");
    }
}
