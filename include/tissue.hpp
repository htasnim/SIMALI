...
inline string view_object_str(ViewObject view_object) {
  switch (view_object) {
    ...
    case ViewObject::EPITYPE:           return "epicell_type"; 
...
struct SampleData {
  ...
  EpiCellType   epicell_type   = EpiCellType::AIR; 
  float virions = 0;
  float chemokine = 0;
};
...
class Tissue {
 ...
 public:
  ...
  void log_epicell_type_counts();

  void write_epicell_chunk_counts_csv(const std::string &fname);
