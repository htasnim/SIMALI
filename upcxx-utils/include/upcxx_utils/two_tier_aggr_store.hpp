...
public:
  ...
  using future_block_t = future< block_t >;
  using promise_block_t = ActiveInstantiationTimer< promise<block_t> >;

  using blocks_t = vector< block_t >;
  using reservation_t = shared_ptr< blocks_t >;
  using promise_reservation_t = ActiveInstantiationTimer< promise<reservation_t> >;
...
protected:
  ...
  reservation_t claim_reservation(bool require_full = true)
  {
    ...
    if (require_full && reservation.size() != reservation_size) DIE("claim_reservation is not full but this is required. ", reservation.size(), "\n");
...
public:
  ...
  bool valid() const
  {
    bool is_valid = (backing.first && backing.second > 0 && num_blocks > 0 && count_per_block > 1 && num_blocks * count_per_block <= backing.second);
    return is_valid;
  }
  ...
  void set(block_t allocation, size_t num, size_t count, size_t thread_offset = 0, size_t reservation_count = 0)
  {
    ...
    if (available.size() + reservation.size() != num_blocks) DIE("Invalid set() - available size is not num_blocks: ", available.size(), " available + ", reservation.size(), " reserved vs ", num_blocks, "\n");
...
public:
  ...
  // non blocking 
  future< reservation_t > acquire_reservation()
  {
  ...
  void push(block_t &block)
  {
    ...
    if (!is_backed(block)) DIE("push called on foreign block(", block.first, " ", block.second, "): ", backing.first, " ", backing.second, "\n");
    ...
    if (available.size() + reservation.size() > num_blocks) DIE("push added too many blocks: " , available.size(), " + ",reservation.size(), " vs " , num_blocks, "\n");
  ...
  inline bool available_empty() const {
  ...
  bool empty() const
  {
    //DBG("empty():  promised reservations=", promised_reservations_count, " promised_blocks_count=", promised_blocks_count, " available=", available.size() , " reserved=", reservation.size(), " num_blocks=", num_blocks, "\n");
  ...
  inline size_t promises_size() const {
    return promised_reservations_count + promised_blocks_count;
  }

  inline bool promises_empty() const {
    return promised_reservations.empty() && promised_blocks.empty();
  }
...
public:
  ...
  using future_src_dest_block_t = future< global_block_t, global_block_t >;
...
public:
  ...
  {
    DBG("FixedMemoryRPC - ", description, "::set_dest_stores(num_stores=", num_stores, ") global_blocks=", global_dispatcher.get_num_blocks(), " count=", global_dispatcher.get_count_per_block(), " avail=", global_dispatcher.available_size(), "\n");
    assert(valid());
    if (global_dispatcher.get_count_per_block() > 1) {
      ...
      if (global_dispatcher.available_size() < num_stores) {
        DIE("There are an insufficient number of available blocks to populate the dest stores: available_blocks=", global_dispatcher.available_size(), ", num_stores=", num_stores, "\n");
      }
  ...
  bool empty() const
  {
    bool is_empty = global_dispatcher.empty()
           && global_dispatcher.available_size() + global_dispatcher.reserved_size() + count_empty_dest_stores() == global_dispatcher.get_num_blocks();
  ...
  void set_fixed_mem(size_t num_global_blocks, size_t count_per_block, size_t num_stores, bool includes_thread_offset = false, size_t num_reserved_blocks = 0)
  {
    ...
    if (num_global_blocks <= num_stores + num_reserved_blocks) { DIE("Invalid set_fixed_mem num_global_blocks=", num_global_blocks, " num_stores=", num_stores, " num_reserved_blocks=", num_reserved_blocks, "\n"); }
    ...
    SOUT("Using ", num_global_blocks, " global of ", count_per_block, " elements (of ", get_size_str(sizeof(T)), ") aggregate & send (", get_size_str(total_global_bytes), ") per thread or ", get_size_str(total_bytes * upcxx::local_team().rank_n()), " per node in shared memory for dest and send buffers\n");
    assert(valid());
  }
  ...
  inline bool has_dest_stores() const {
    return dest_stores.size() > 0;
  }
  ...
  global_block_t &dest_store(size_t store_idx)
  {
    ...
    if (dest_stores.size() <= store_idx) DIE("There are no dest stores at the moment:", dest_stores.size(), " looking for ", store_idx, "\n");
  ...
  inline future< global_reservation_t > acquire_reservation() {
    return global_dispatcher.acquire_reservation();
  }

  inline global_reservation_t acquire_partial_reservation() {
    return global_dispatcher.acquire_partial_reservation();
  }

  inline void release_reservation( global_reservation_t reserved ) {
    global_dispatcher.release_reservation( reserved );
  }

  inline bool has_promises() const {
    return !global_dispatcher.promises_empty();
  }

  inline size_t global_available_size() const {
    return global_dispatcher.available_size();
  }

  inline size_t global_reserved_size() const {
    return global_dispatcher.reserved_size();
  }
  ...
  future_src_dest_block_t rput_block(global_block_t &src, global_block_t &dest)
  {
    ...
    return fut_return.then(
      [rput_t, send_offset](global_block_t src, global_block_t dest)
      {
        size_t count = dest.second + send_offset;
        DBG("rput completed ", dest.second, " elements with ", send_offset, " extra (", get_size_str(count * sizeof(T)), ") src=", src.first, " dest=", dest.first, " in ", rput_t->get_elapsed_since_start(), " s, ", get_size_str(count * sizeof(T) / rput_t->get_elapsed_since_start()), " / s\n");
  ...
  future_src_dest_block_t rget_block(global_block_t &src, global_block_t &dest)
  {
    ...
    return fut_return.then(
      [rget_t, send_offset](global_block_t src, global_block_t dest)
      {
        size_t count = dest.second + send_offset;
        DBG("rget completed ", dest.second, " elements with ", send_offset, " extra (", get_size_str(count * sizeof(T)), ") src=", src.first, " dest=", dest.first, " in ", rget_t->get_elapsed_since_start(), " s, ", get_size_str(count * sizeof(T) / rget_t->get_elapsed_since_start()), " / s\n");
        return make_future(src,dest);
      });
  }
  ...
  future_src_dest_block_t rget_block(global_block_t &gblock)
  {
    ...
    // rget the block
    auto fut_blocks = fut_both.then(
                         [rget_wait_t](global_block_t src, global_block_t dest)
                         {
                            // just stop the timer
                            return make_future(src,dest);
                         }).then(
                         [this](global_block_t src, global_block_t dest)
                         {
                            return this->rget_block(src, dest);
                         }
                      );
    return fut_blocks;
  }
  ...
  inline size_t get_thread_offset() const {
...
private:
  ...
  size_t max_rpcs_in_flight; // Limit for the number of rpcs in flight. This limit exists to prevent the dispatch buffers from growing indefinitely
...
// private static methods
  ...
  inline bool &my_progress_required() {
  ...
  bool calc_my_progress_required()
  {
    ...
    } else if (track_inter_rpcs->get_rpc_inner_timer().get_total_count() + track_inter_rpcs->get_rpc_inner_timer().get_active_count() < track_inter_rpcs->get_rpc_timer().get_total_count() + track_inter_rpcs->get_rpc_timer().get_active_count()) {
  ...
  static int block_quicksort_partition (T * block, int low, int high)
  {
    ...
    for (int j = low; j <= high - 1; j++) {
        ...
        if (get_thread_from_block(block, j) < pivot) {
            ...
            assert(i >= low); assert(j >= low);
            assert(i < high); assert(j < high);
            if (i != j) {
              assert(i<j);
  ...
  static vector<intra_global_block_t> inter_to_sorted_intra_blocks(inter_global_block_t &gblock, size_t start = 0)
  {
    ...
    for (size_t idx = start; idx < gblock.second; idx++) {
      ...
      if (last_thread > thread) DIE("inter_to_sorted_intra_blocks did not sort properly. idx=", idx, " gblock.second=", gblock.second, " last_thread=", (int) last_thread, " thread=", (int) thread, "\n");
  ...
  static future<inter_global_block_t> inter_intra_inner_rpc_relay(inter_global_block_t lblock, track_rpcs_t &track_inter_rpcs, track_rpcs_t &track_intra_rpcs, intra_fixed_memory_t &intra_fixed_mem, inter_fixed_memory_t &inter_fixed_mem, FuncDistObj &func)
  {
    ...
    future<intra_global_block_t> all_rpcs = make_future(lblock_consumed);
    ...
    if (idx < lblock.second) {
      ...
      for (intra_global_block_t intra_block : intra_blocks) {
        ...
        if (intra_block.first && intra_block.second > 0) {
          ...
          auto fut_rpc = just_send_intra_rpc_nb(split_rank::get_rank_from_thread(thread_idx), intra_block, intra_fixed_mem, track_intra_rpcs, func).then(
                         [](intra_global_block_t ignored) {});
          all_rpcs = when_all(all_rpcs, fut_rpc);
          direct_sent++;
        }
        thread_idx++;
      }
      all_rpcs = all_rpcs.then(
                 [inter_intra_rpc_timer, direct_sent](inter_global_block_t lblock)
                 {
                   DBG("inter_relay All direct_sent=", direct_sent, " intra rpcs relayed from inter rpc took ", inter_intra_rpc_timer->get_elapsed_since_start(), " s count=", inter_intra_rpc_timer->get_total_count(), " active=", inter_intra_rpc_timer->get_active_count(), "\n");
                   assert(lblock.first);
                   assert(lblock.first.is_local());
  ...
  void send_inter_rpc(intrank_t target_rank, inter_global_block_t &gblock)
  {
    ...
    auto fut = rpc(target_rank,
                   [](inter_global_block_t gblock, track_rpcs_t &track_inter_rpcs, track_rpcs_t &track_intra_rpcs, intra_fixed_memory_t &intra_fixed_mem, inter_fixed_memory_t &inter_fixed_mem, FuncDistObj &func)
                   {
                     assert(gblock.first.where() != rank_me()); // no local data should be transmitted via RPC 
                     assert(split_rank::get_my_node() != split_rank::get_node_from_rank(gblock.first.where())); // no inter-node transfer on the same node
                     ...
                     auto fut_relay = fut_blocks.then(
                         [&track_inter_rpcs, &track_intra_rpcs, &inter_fixed_mem, &intra_fixed_mem, &func](inter_global_block_t gblock_IGNORED, inter_global_block_t lblock)
                         {
                           assert(gblock_IGNORED.first);
                           assert(gblock_IGNORED.first.where() != rank_me());
                           assert(gblock_IGNORED.second == 0); // is drained
                           return inter_intra_inner_rpc_relay(lblock, track_inter_rpcs, track_intra_rpcs, intra_fixed_mem, inter_fixed_mem, func).then(
                             [&inter_fixed_mem](inter_global_block_t lblock)
                             {
                               assert(lblock.first);
                               assert(lblock.first.is_local());
                         ...
                         [t_inner_rpc]()
                         {
                           DBG("Completed inter rpc relay in ", t_inner_rpc->get_elapsed_since_start(), " s count=", t_inner_rpc->get_total_count(), " active=", t_inner_rpc->get_active_count(), "\n");
                           // stop the timer
                         });
                      // no need to wait on fut_relay -- cleanup of the timer and push back to inter_fixed_mem will eventually happen and be verified

                      // future for gblock after it is consumed -- possibly with a return package
                      auto fut_return = fut_blocks.then(
                         [&inter_fixed_mem](inter_global_block_t gblock, inter_global_block_t lblock_ignored) -> future<inter_global_block_t>
                         {
                           ...
                           if (inter_fixed_mem->has_dest_stores() && inter_fixed_mem->global_reserved_size() > 0) {
                             ...
                             if (store_block.first && store_size * 2 > inter_fixed_mem->get_count_per_block() && store_size * sizeof(T) > 4*ONE_KB) {
                               // send this store back to sender with this gblock
                               DBG("Sending store_size=", store_size, " back to ", gblock.first.where(), " store=", store_block.first, " (swapped)\n");
                               ...
                               return fut_rput_blocks.then(
                                  [&inter_fixed_mem](inter_global_block_t lblock, inter_global_block_t gblock)
                                  {
                                    DBG("rput finished lblock=", lblock.first, " ", lblock.second, " gblock=", gblock.first, " ", gblock.second, "\n");
                                    assert(lblock.first);
                                    assert(lblock.first.where() == rank_me());
                         ...
                         [t_inner_rpc](inter_global_block_t gblock)
                         {
                           DBG("Returning inter rpc gblock=", gblock.first, " ", gblock.second, " in ", t_inner_rpc->get_elapsed_since_start(), "\n");
                           return gblock;
                         });
    ...
    auto fut_returned = fut.then(
                 [t_rpc, target_rank](inter_global_block_t gblock)
                 {
                 ...
                 [&inter_fixed_mem, &_track_inter_rpcs, &_track_intra_rpcs, &intra_fixed_mem, &_func](inter_global_block_t gblock) -> future<inter_global_block_t>
                 {
                   assert(gblock.first);
                   assert(gblock.first.where() == rank_me());
                   ...
                   if (gblock.second > 0) {
                     ...
                     return inter_intra_inner_rpc_relay(gblock, _track_inter_rpcs, _track_intra_rpcs, intra_fixed_mem, inter_fixed_mem, _func);
                   } else {
                     return to_future(gblock);
                   }
                 }).then(
                 [&inter_fixed_mem](inter_global_block_t gblock)
                 {
                   assert(gblock.first);
                   assert(gblock.second == 0); // it is drained
  ...
  static void send_intra_rpc(intrank_t target_rank, intra_global_block_t &gblock, track_rpcs_t &track_intra_rpcs, intra_fixed_memory_t &intra_fixed_mem, FuncDistObj &func)
  {
    assert(!gblock.first.is_null());
    assert(gblock.first.where() == rank_me());
    assert(gblock.second > 0);
    ...
    // handle returned global_block
    auto fut_returned = fut_gblock.then(
                 [&intra_fixed_mem](intra_global_block_t gblock)
                   {
  ...
  static future<intra_global_block_t> just_send_intra_rpc_nb(intrank_t target_rank, intra_global_block_t &gblock, intra_fixed_memory_t &intra_fixed_mem, track_rpcs_t &track_intra_rpcs, FuncDistObj &func)
  {
    // time the round-trip
    ...
    auto fut = rpc(target_rank,
                   [](intra_global_block_t gblock, FuncDistObj &func, intra_fixed_memory_t &intra_fixed_mem, track_rpcs_t &track_intra_rpcs)
                   {
                     ...
                     future< intra_global_block_t > fut_gblock;
  ...
  inline void update_remote1_nb(intrank_t target_rank, const T &elem) {
    send_rpc1(target_rank, elem);
  }


  // operate on a vector of elements in the dest_stores
  // will block until sufficient global available blocks are available
  inline static void update_remote_intra(intrank_t target_rank, intra_global_block_t &gblock, track_rpcs_t &track_intra_rpcs, intra_fixed_memory_t &intra_fixed_mem, FuncDistObj &func) {
    intra_reservation_t empty_res;
    update_remote_intra_nb(target_rank, gblock, track_intra_rpcs, intra_fixed_mem, empty_res, func);
  }

  // and subject to the maximum rpcs in flight
  static void update_remote_intra_nb(intrank_t target_rank, intra_global_block_t &gblock, track_rpcs_t &track_intra_rpcs, intra_fixed_memory_t &intra_fixed_mem, intra_reservation_t &reservation, FuncDistObj &func)
  {
  ...
  static future<> replace_inter_store(inter_global_block_t &gblock, inter_fixed_memory_t &inter_fixed_memory_store)
  {
    assert(!gblock.first);
    assert(gblock.second == 0);
    future< inter_global_block_t > newblock = inter_fixed_memory_store->pop_global();
    return when_all(make_future(std::ref(gblock)), newblock).then(
      [&inter_fixed_memory_store](inter_global_block_t &gblock, inter_global_block_t newblock)
        {
  ...
  static future<> replace_intra_store(intra_global_block_t &gblock, intra_fixed_memory_t &intra_fixed_memory_store)
  {
    future< intra_global_block_t > newblock = intra_fixed_memory_store->pop_global();
    return when_all(make_future(std::ref(gblock)), newblock).then(
      [&intra_fixed_memory_store](intra_global_block_t &gblock, intra_global_block_t newblock)
        {
          assert(newblock.first);
          assert(newblock.first.where() == rank_me());
          assert(newblock.second == 0);
  ...
  static void replace_intra_store_nb(intra_global_block_t &gblock, intra_fixed_memory_t &intra_fixed_mem, intra_reservation_t &reservation)
  {
    assert(reservation);
  ...
  future<> update_remote_inter_nb(intrank_t target_rank, inter_global_block_t &gblock)
  {
  ...
  inline static bool add_to_dest_store_intranode(split_rank split, const T &elem, track_rpcs_t &track_intra_rpcs, intra_fixed_memory_t &intra_fixed_mem, FuncDistObj &func) {
    intra_reservation_t empty_res;
  ...
  static bool add_to_dest_store_intranode_nb(split_rank split, const T &elem, track_rpcs_t &track_intra_rpcs, intra_fixed_memory_t &intra_fixed_mem, intra_reservation_t &reservation, FuncDistObj &func)
  {
    // intranode
    ...
    if (gblock.second >= max_store_size) DIE("Invalid state of gblock with ", gblock.second, " elements but max of ", max_store_size, "\n");
    assert(gblock.first);
    assert(gblock.first.where() == rank_me());
    ...
    if (gblock.second == max_store_size) {
       //DBG("add_to_dest_store_intranode found full for ", (int) split.get_thread(), "/", split.get_rank(), " reservation:", reservation.get(), ", gblock=", gblock.first, ",", gblock.second, "\n");
...
public:
  ...
  string to_string() const
  {
    ...
    os << "inter_rpc_t=" << track_inter_rpcs->get_rpc_timer().get_total_count() << "/" << track_inter_rpcs->get_rpc_inner_timer().get_total_count() << ",";
    os << "inter_intra_rpc_t=" << track_inter_rpcs->get_rpc_relay_timer().get_total_count() << ",";
    os << "track_intra_rpcs=" << track_intra_rpcs->to_string() << ",";
    os << "intra_rpc_t=" << track_intra_rpcs->get_rpc_timer().get_total_count() << "/" << track_intra_rpcs->get_rpc_inner_timer().get_total_count() << ",";
    os << ")";
    return os.str();
  }
  ...
  static void optimal_num_blocks_and_count_per(const size_t max_bytes, const size_t max_rpcs, size_t &num_intra_blocks, size_t &num_inter_blocks, size_t &count_per_block)
  {
    ...
    size_t min_intra_rpcs_in_flight = intra_dest_store_size + min_reservations * res_size + 16; // every dest store + a few reservations + 16
    ...
    DBG("optimizing max_bytes=", get_size_str(max_bytes), " min_rpcs=", min_rpcs_in_flight, " inter=", inter_dest_store_size, " intra=", intra_dest_store_size, "\n");
    if (max_bytes >= 2 * sz * (min_rpcs_in_flight + inter_dest_store_size + intra_dest_store_size) ) {
      ...
      DBG("optimizing mem_per_block=", get_size_str(mem_per_block), " rpcs=", rpcs_in_flight, " count_per_block=", count_per_block, " block_size=", get_size_str(count_per_block*sz), "\n");
      do {
    ...
    if (num_blocks > 1) {
      ...
      if (split_rank::num_nodes() >= 1) {
        num_inter_blocks = inter_dest_store_size + min_inter_rpcs_in_flight + (rpcs_in_flight - min_rpcs_in_flight) * inter_fraction;
      } else {
        num_inter_blocks = 0;
        inter_fraction = 0.0;
      }
      num_intra_blocks = intra_dest_store_size + min_intra_rpcs_in_flight + (rpcs_in_flight - min_rpcs_in_flight) * (1.0-inter_fraction);
      assert(num_blocks >= num_inter_blocks + num_intra_blocks);

    } else {
      num_intra_blocks = num_inter_blocks = 0;
    }
    SOUT("Calculated optimal num and block size for ", split_rank::num_nodes(), " internode sets and ", (size_t) split_rank::num_threads(), " intranode ranks per node\n");
  ...
  void set_size(size_t max_store_bytes)
  {
    ...
    if (count_per_block <= 1) {
      ...
      if (max_store_bytes > 0) {
        ...
        SWARN("Using no TwoTierAggrStore to aggregate messages because no configutation works with less than ", get_size_str(max_store_bytes), " at this scale\n");
      }
    } else {
    ...
    intra_fixed_memory_store->set_fixed_mem(num_intra_blocks, count_per_block, split_rank::num_threads(), false, nodes == 1 ? 0 : split_rank::num_threads());
    ...
    if (nodes == 1) {
      ...
      inter_fixed_memory_store->set_fixed_mem(0, 0, 1, false, 0);
    } else {
      if (num_inter_blocks > 0) assert(num_inter_blocks >= split_rank::num_nodes() * 3); // room for dest store, 1 reservation and 1 in flight
      inter_fixed_memory_store->set_fixed_mem(num_inter_blocks, count_per_block, split_rank::num_nodes(), true, split_rank::num_nodes());
    }
    ...
    SOUT("Using a ", description, " store of max ",
         ...
         get_size_str(per_inter_rpc_bytes * num_inter_blocks), ") and ", max_rpcs_in_flight,
         " rpcs in flight\n");
  }
  ...
  inline bool empty() const {
    return track_intra_rpcs->empty() && intra_fixed_memory_store->empty() && track_inter_rpcs->empty() && inter_fixed_memory_store->empty();
  }
  ...
  void flush_inter_updates()
  {
    ...
    for (node_num_t _node = 0 ; _node < num_inter_dest; _node++) {
      node_num_t node = (_node + 1 + split_rank::get_my_node()) % split_rank::num_nodes(); // rotate the flushes, starting with the next node in the job
      if (max_store_size > 0) {
  ...
  void flush_intra_updates()
  {
    ...
    for (thread_num_t _thread = 0 ; _thread < split_rank::num_threads(); _thread++) {
      thread_num_t thread = (_thread + 1 + split_rank::get_my_thread()) % split_rank::num_threads(); // rotate the flushes starting with the next thread
      if (max_store_size > 0) {
  ...
  static void flush_intra_updates_with_res(intra_reservation_t &reservation, track_rpcs_t &track_intra_rpcs, intra_fixed_memory_t &intra_fixed_mem, FuncDistObj &func)
  {
    assert(reservation);
    int count_flushed = 0;
    if (!reservation->empty()) {
      ...
      while ( !reservation->empty() && !rank_counts.empty() ) {
        size_t r_c = rank_counts.back(); rank_counts.pop_back();
        ...
        assert(gblock.first);
        assert(gblock.first.where() == rank_me());
        update_remote_intra_nb(split_rank::get_rank_from_thread(thread), gblock, track_intra_rpcs, intra_fixed_mem, reservation, func);
        assert(gblock.first);
        assert(gblock.first.where() == rank_me());
        count_flushed++;
      }
  ...
  void flush_updates()
  {
    ...
    do {
      ...
      my_progress();
      while (res->size() < split_rank::num_threads() && intra_fixed_memory_store->global_available_size() > split_rank::num_threads()) {
